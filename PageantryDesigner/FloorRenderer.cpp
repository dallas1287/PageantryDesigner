...
FloorRenderer::FloorRenderer(GraphicsPanel* parent) : m_parent(parent), m_ebo(QOpenGLBuffer(QOpenGLBuffer::IndexBuffer))
{
	initialize();
}
...
void FloorRenderer::initTexture(const QString& path)
{
...
void FloorRenderer::drawFloor()
{
	...
	m_floorProgram->setAttributeBuffer(m_texCoordAttr, GL_FLOAT, sizeof(decltype(vertices->m_position)), 2, sizeof(VertexData));

	m_floorProgram->enableAttributeArray(m_colAttr);
	m_floorProgram->setAttributeBuffer(m_colAttr, GL_FLOAT, sizeof(decltype(vertices->m_position)) + sizeof(decltype(vertices->m_texCoord)), 3, sizeof(VertexData));
	m_vbo.release();
...
void FloorRenderer::drawGridLines()
{
	...
	VertexData::makeGrid(4, &vertices, vSize, &indices, iSize);
